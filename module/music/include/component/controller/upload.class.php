 ...
 * @version 		$Id: upload.class.php 3547 2011-11-22 14:00:19Z Raymond_Benc $
 */
class Music_Component_Controller_Upload extends Phpfox_Component
{
	...
	public function process()
	{
			...
			if ($sMethod == 'simple' && !empty($aVals))
			{
				...
				if ($oValidator->isValid($aVals))
				{
					...
					else 
					{
						...
						{
							echo '<script type="text/javascript">';
							echo 'console.log("Error check 1");';
							echo '</script>';
						}
					}
				}
		...
		$this->template()->setTitle(($bIsEdit ? 'Editing Song: ' . $aEditSong['title'] : Phpfox::getPhrase('music.upload_a_song')))
			->setBreadcrumb(Phpfox::getPhrase('music.music'), ($aCallback === false ? $this->url()->makeUrl('music') : $aCallback['url_home_photo']))
			->setBreadcrumb(($bIsEdit ? 'Editing Song: ' . $aEditSong['title'] : Phpfox::getPhrase('music.upload_a_song')), $this->url()->makeUrl('music.upload'), true)
			->setFullSite()
			->setPhrase(array(
					'music.select_an_mp3'
				)
