...
class Music_Service_Process extends Phpfox_Service 
{
	...
	public function delete($iId, &$aSong = null)
	{
		...
		{
			$bSkip = false;
			$aSong = $this->database()->select('song_id, module_id, item_id, user_id, song_path, is_sponsor')
				->from($this->_sTable)
	...
	public function approve($iId)
	{
		...
		{			
			...
			(Phpfox::isModule('feed') ? Phpfox::getService('feed.process')->add('music_album', $aSong['song_id'], $aSong['album_privacy'], (isset($aSong['album_privacy_comment']) ? (int) $aSong['album_privacy_comment'] : 0), 0, $aSong['user_id']) : null);
		}
		else 
		{
			(Phpfox::isModule('feed') ? Phpfox::getService('feed.process')->add('music_song', $aSong['song_id'], $aSong['privacy'], (isset($aSong['privacy_comment']) ? (int) $aSong['privacy_comment'] : 0), 0, $aSong['user_id']) : null);
		}		
