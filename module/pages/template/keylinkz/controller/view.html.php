...
<div class="item_bar">
	<div class="item_bar_action_holder">
		{if $aPage.view_id == '1' && Phpfox::getUserParam('pages.can_moderate_pages')}
			...
			<a href="#" class="item_bar_approve" onclick="$(this).hide(); $('#js_item_bar_approve_image').show(); $.ajaxCall('pages.approve', 'page_id={$aPage.page_id}'); return false;">{phrase var='pages.approve'}</a>
		{/if}		
		<a href="#" class="item_bar_action"><span>{phrase var='pages.actions'}</span></a>		
		<ul>
...
<div id="js_pages_user_entry_{$aPendingUser.signup_id}" class="row1{if $phpfox.iteration.pendingusers == 1} row_first{/if}">
	...
	<div style="margin-left:55px">
		<span class="row_title_link">{$aPendingUser|user}</span>
	</div>
	<div class="clear"></div>
</div>
{/foreach}
