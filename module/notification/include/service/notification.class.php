...
class Notification_Service_Notification extends Phpfox_Service 
{
	...
	public function getForBrowse($iPage = 0, $iPageTotal = 10)
	{
		...
		foreach ($aRows as $aRow)
		{
			...
			{
				 $iDate = Phpfox::getTime('F d', $aRow['time_stamp']);
			}						
			
			if (substr($aRow['type_id'], 0, 8) != 'comment_' && !Phpfox::hasCallback($aRow['type_id'], 'getNotification'))
			{				
				$aRow['link'] = '#';
				$aRow['message'] = 'Notification is missing a callback. [' . $aRow['type_id'] . '::getNotification]';
	...
	public function get()
	{
		...
		foreach ($aRows as $aRow)
		{
			...
			{					
				...
				{
					$aCallBack['link'] = '#';
					$aCallBack['message'] = 'Notification is missing a callback. [' . $aRow['type_id'] . '::getNotification]';										
				}		
	...
	public function getUsers($aNotification, $bRemove = false)
	{
		if (isset($aNotification['extra_users']) && is_array($aNotification['extra_users']) && count($aNotification['extra_users']))
		{
			...
			{	
				...
				$sStr .= '<span class="drop_data_user">' . ((isset($aUser['user_id']) && $aUser['user_id'] == Phpfox::getUserId()) ? Phpfox::getPhrase('notification.you') : $aUser['full_name']) . '</span>, ';
			}
			...
			else 
			{
				if (isset($sExtraTotalUsers))
				{
					if ($sExtraTotalUsers === 1)
					{
						$sStr = $sStr . ' <span class="drop_data_user">' . ((isset($aNotification['user_id']) && $aNotification['user_id'] == Phpfox::getUserId()) ? 'You' : $aNotification['full_name']) . '</span> ' . Phpfox::getPhrase('notification.and') . ' ' . $sExtraTotalUsers . ' ' . Phpfox::getPhrase('notification.other') . ' ';
					}
					else 
					{
						$sStr = $sStr . ' <span class="drop_data_user">' . ((isset($aNotification['user_id']) && $aNotification['user_id'] == Phpfox::getUserId()) ? 'You' : $aNotification['full_name']) . '</span> ' . Phpfox::getPhrase('notification.and') . ' ' . $sExtraTotalUsers . ' ' . Phpfox::getPhrase('notification.others') . ' ';
					}				
				}
				else 
				{
					$sStr = (empty($sStr) ? '' : rtrim($sStr, ', ') . ' ' . Phpfox::getPhrase('notification.and')) . ' <span class="drop_data_user">' . ((isset($aNotification['user_id']) && $aNotification['user_id'] == Phpfox::getUserId()) ? 'You' : $aNotification['full_name']) . '</span>';
				}
			}
		...
		$sStr = '<span class="drop_data_user">' . ((isset($aNotification['user_id']) && $aNotification['user_id'] == Phpfox::getUserId()) ? Phpfox::getPhrase('notification.you') : $aNotification['full_name']) . '</span>';
