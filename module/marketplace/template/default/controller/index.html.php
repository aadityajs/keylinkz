 ...
 * @author  		Raymond Benc
 * @package 		Phpfox
 * @version 		$Id: index.html.php 3346 2011-10-24 15:20:05Z Raymond_Benc $
 */
...
{/if}
				<div class="row_title">				
					...
					{/if}						
							...
							{if Phpfox::getUserParam('event.can_approve_events') || Phpfox::getUserParam('event.can_delete_other_event')}<a href="#{$aListing.listing_id}" class="moderate_link" rel="marketplace">Moderate</a>{/if}
						</div>
						<div class="row_title_info">		
							
								<a href="{$aListing.url}" class="link" title="{$aListing.title|clean}">{$aListing.title|clean|shorten:100:'...'|split:25}</a>{if $aListing.view_id == '2'}<span class="marketplace_item_sold">({phrase var='marketplace.sold'})</span>{/if}
							
	<div class="marketplace_price_tag">
		    ...
		    {/if}
							...
							<div class="item_content">
								{$aListing.description|parse|highlight:'search'|split:25|shorten:200:'marketplace.see_more':true}				
							</div>							
