...
?>
					...
					{if isset($bBlogView)}
						<li class="item_delete"><a href="{url link="blog.delete" id=""$aItem.blog_id""}" onclick="return confirm('{phrase var='blog.are_you_sure_you_want_to_delete_this_blog' phpfox_squote=true}');" title="{phrase var='blog.delete_blog'}">{phrase var='core.delete'}</a></li>
					{else}
						<li class="item_delete"><a href="#TB_inline?type=delete&amp;itemId={$aItem.blog_id}&amp;call=blog.inlineDelete" class="thickbox" title="{phrase var='blog.delete_blog'}">{phrase var='core.delete'}</a></li>
					{/if}
					{/if}
