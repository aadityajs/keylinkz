...
class Blog_Service_Callback extends Phpfox_Service 
{
	...
	public function getActivityFeed($aRow)
	{
		...
		$aRow = $this->database()->select('b.blog_id, b.title, b.time_stamp, b.total_comment, b.total_like, bt.text_parsed AS text')
	...
	public function addLike($iItemId, $bDoNotSendEmail = false)
	{
		...
		if (!$bDoNotSendEmail)
		{
			...
			Phpfox::getLib('mail')->to($aRow['user_id'])
				->subject(Phpfox::getPhrase('blog.full_name_liked_your_blog_title', array('full_name' => Phpfox::getUserBy('full_name'), 'title' => $aRow['title'])))
				->message(Phpfox::getPhrase('blog.full_name_liked_your_blog_link_title', array('full_name' => Phpfox::getUserBy('full_name'), 'link' => $sLink, 'title' => $aRow['title'])))
	...
	public function getTagLink()
	{
		...
		return Phpfox::getLib('url')->makeUrl('blog.tag');
	}
	...
	public function updateCounter($iId, $iPage, $iPageLimit)
	{
		...
		elseif ($iId == 'blog-activity')
		{
			...
			$aRows = $this->database()->select('m.user_id, m.activity_blog, m.activity_points, m.activity_total, COUNT(oc.blog_id) AS total_items')
			...
			foreach ($aRows as $aRow)
			{
				$this->database()->update(Phpfox::getT('user_activity'), array(
					'activity_points' => (($aRow['activity_total'] - ($aRow['activity_points'] * Phpfox::getUserParam('blog.points_blog'))) + ($aRow['total_items'] * Phpfox::getUserParam('blog.points_blog'))),
	...
	public function getTotalItemCount($iUserId)
	{
		return array(
			'field' => 'total_blog',
			'total' => $this->database()->select('COUNT(*)')->from(Phpfox::getT('blog'))->where('user_id = ' . (int) $iUserId . ' AND is_approved = 1 AND post_status = 1')->execute('getSlaveField')
		);	
	}
