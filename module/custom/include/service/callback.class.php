...
class Custom_Service_Callback extends Phpfox_Service 
{
	...
	public function addLike($iItemId, $bDoNotSendEmail = false)
	{
		...
		if (!$bDoNotSendEmail)
		{
			...
			Phpfox::getLib('mail')->to($aFeed['user_id'])
					->subject(Phpfox::getPhrase('custom.full_name_liked_your_change_on_phrase_var_name', array('full_name' => Phpfox::getUserBy('full_name'), 'phrase_var_name' => Phpfox::getPhrase($aRow['phrase_var_name']))))
					->message(Phpfox::getPhrase('custom.full_name_liked_your_change_on_message', array('full_name' => Phpfox::getUserBy('full_name'), 'link' => $sLink, 'phrase_var_name' => Phpfox::getPhrase($aRow['phrase_var_name']))))
	...
	public function getActivityFeedRelation($aFeed)
	{
		...
		$sLink = Phpfox::getLib('url')->makeUrl('friend');
	...
	public function addLikeRelation($iItemId, $bDoNotSendEmail = false)
	{
		...
		// if (!$bDoNotSendEmail)
		{
			...
			Phpfox::getLib('mail')->to($aRow['user_id'])
					->subject(Phpfox::getPhrase('custom.full_name_liked_your_change_in_relationship_title', array('full_name' => Phpfox::getUserBy('full_name'), 'title' => $aRow['title'])))
					->message(Phpfox::getPhrase('custom.full_name_liked_your_change_in_relationship_message', array('full_name' => Phpfox::getUserBy('full_name'), 'link' => $sLink, 'title' => $aRow['title'])))
					->notification('like.new_like')
					->send();
