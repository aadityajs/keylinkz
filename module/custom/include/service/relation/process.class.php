...
class Custom_Service_Relation_Process extends Phpfox_Service
{
	...
	public function updateRelationship($iRelationType, $iWith = null, $iUser = null)
	{
		...
		if (!empty($aExisting['relation_data_id']) && $aExisting['status_id'] == 1)
		{
			...
			Phpfox::getLib('mail')->to(array($iUser, $iWith))
				->subject(Phpfox::getPhrase('custom.relationship_status_confirmed'))
				->message(Phpfox::getPhrase('custom.full_name_and_full_name_with_are_now_phrase_var_name', array('full_name' => Phpfox::getUserBy('full_name'), 'full_name_with' => $aExisting['full_name_with'], 'phrase_var_name' => Phpfox::getPhrase($aExisting['phrase_var_name']))))
				->send();
		...
		if ($iWith != $iUser)
		{
			...
			Phpfox::getLib('mail')->to(array($iUser, $iWith))
				->subject('Relationship Status Confirmation')
				->message(Phpfox::getUserBy('full_name') . ' wants to list you both as "' . Phpfox::getPhrase($aExisting['phrase_var_name']) . '".' . "\nThis view all your relationship requests follow the link below:\n<a href=\"" . $sLink . "\">" . $sLink . "</a>")
				->send();
		}
	 ...
	 * @return boolean
	 */
	public function _denyStatus($iStatusId, $iUser = null)
	{
