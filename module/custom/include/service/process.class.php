 ...
 * @version 		$Id: process.class.php 3730 2011-12-08 11:08:14Z Miguel_Espinoza $
 */
...
{	
	...
	{		
		...
		$sCustomTable = 'user_custom';
		$sCustomValueTable = 'user_custom_value';		
		...
		if (!$this->database()->tableExists(Phpfox::getT($sCustomTable)))
		{
			return Phpfox_Error::set(Phpfox::getPhrase('custom.table_does_not_exist', array('sTableName' => $sCustomTable)));
		}
		if ($this->database()->isField(Phpfox::getT($sCustomTable), Phpfox::getService('custom')->getAlias() . $sFieldName))
		{
		...
		/*$this->database()->addField(array(
				'table' => Phpfox::getT($sCustomTable),
		...
		if ( $bAddField && 
			!$this->database()->isField(Phpfox::getT($sCustomValueTable), Phpfox::getService('custom')->getAlias() . $sFieldName))
		{
			$this->database()->addField(array(
					'table' => Phpfox::getT($sCustomValueTable),
					'field' => Phpfox::getService('custom')->getAlias() . $sFieldName,
					'type' => $sValueTypeName
				)
			);		
	...
	public function delete($iId)
	{
		...
		{
			if (Phpfox::getUserGroupParam($aField['user_group_id'], 'custom.has_special_custom_fields'))
			{
				$sTable = Phpfox::getUserGroupParam($aField['user_group_id'], 'custom.custom_table_name');
