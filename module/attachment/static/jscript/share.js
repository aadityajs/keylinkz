...
$Core.shareInlineBox = function($oObj, $sAttachmentId, $bIsInlineAttachment, $sRequest, $iWidth, $sExtra)
{
	...
	if ($bIsInlineAttachment)
	{
		...
		if ($('#global_attachment_list_inline').length <= 0)
		{
			...
			$sContent += '<div id="global_attachment_list_inline_close"><a href="#" onclick="$Core.clearInlineBox(); return false;">Close</a></div>';
			$sContent += '</div>';
		...
		$($oObj).parents('.global_attachment_list:first li a').removeClass('active');
...
$Core.uploadNewAttachment = function($oObj, $bIsMultiShare)
{			
	...
	$oPostParent.find('.js_upload_form_image_holder').find('span:first').html('Uploading ' + $($oObj).val() + '...');
