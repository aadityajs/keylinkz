...
<?php
//echo '<pre>'.print_r($this->getVar('aUser')).'</pre>';
$users= $this->getVar('aUser');
//var_dump($users[1][user_id]);
$usersId = $users['user_id'];

			$userActPointSql = "SELECT * FROM  `keylinkz_user_activity` WHERE user_id = $usersId";
	 		$userActPointRes = mysql_fetch_array(mysql_query($userActPointSql));
			$actPoint = $userActPointRes['activity_points'];

			$userCfValSql = "SELECT * FROM keylinkz_user_custom_value WHERE user_id = $usersId";
			$userCfValRes = mysql_fetch_array(mysql_query($userCfValSql));
			$userCfValInterest = $userCfValRes['cf_interests'];

			$userListingSql = "SELECT COUNT(*) FROM keylinkz_realestate WHERE agent_id = $usersId";
			$userListingRes = mysql_fetch_array(mysql_query($userListingSql));
			$userListingProp = $userListingRes[0];

?>

	<div style="width: 100%; height: 100px; border-bottom: 1px dashed #ebebeb; margin-bottom: 5px; padding-bottom:8px">
	<div class="go_left js_parent_user" style="{if Phpfox::isMobile()}width:44%;{else}width:95%;{/if} float: none;"  id="js_parent_user_{$aUser.user_id}">
		{img user=$aUser suffix='_100_square' max_width=100 max_height=100 class='js_mp_fix_width'}
		<div class="user_browse_user"><?php echo '<span style="position: absolute; left: 157px; ">'.adiGetProfileBadge($users[user_group_id]).'</span>'; ?>
			{$aUser|user|split:25}
			{if Phpfox::isModule('friend')}
			{if $aUser.mutual_friends > 0}
			<div class="user_browse_mutual_friend fr"  style="margin-top: -10px;">
			...
			<div class="user_browse_add_friend fr" style="margin-top: -10px;">
		...
		<!-- PROFILE RATING SECTION -->
          <div style="padding-left:3px; width: 85%; float: right; position: relative; top: -100px;" class="">
              ...
              <div style="height:21px;">
				  ...
				  <div style="padding-top: 2px; padding-left: 20px; float: left; width: 100px;"><a href="#">Write a Review</a></div>
              </div>

              <div style="height:21px; border:">
                  <div style="width:100px;float:left;margin-top:0px; border:">Contributions :</div>
                  <div style="float:left; margin-left:16px;"><a href="#">[<?php echo $actPoint; ?>]</a></div>
              </div>

          </div>

          <!-- PROFILE RATING SECTION -->
			<div class="" style="padding-left:3px; width: 85%; float: right; position: relative; top: -100px;">On Keylinkz: <?php echo $userListingProp; ?> Listings </div>
			<div class="" style="padding-left:3px; width: 85%; float: right; position: relative; top: -100px;">Specialities: <?php echo $userCfValInterest; ?></div>
