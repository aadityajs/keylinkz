...
class User_Component_Controller_Browse extends Phpfox_Component
{
    ...
    public function process()
    {
		...
		if (($sGender = $this->request()->get('gender')))
		{
		    $oFilter->setCondition('AND u.gender = \'' . Phpfox::getLib('database')->escape($sGender) . '\'');
		}

		if (($sLocationChild = $this->request()->get('state')))
		{
		...
		{
		    $oFilter->setCondition('AND ufield.city_location = \'' . Phpfox::getLib('database')->escape(urldecode($sLocationCity)) . '\'');
		}
		...
		if ($this->request()->get('featured') == 1)
		{
		    $this->template()->setHeader(array(
				...
				'<script type="text/javascript">Core_drag.init({table: \'#js_drag_drop\', ajax: \'user.setFeaturedOrder\'});</script>'
				)
			)
    	...
    	if (defined('PHPFOX_IS_ADMIN_SEARCH')) {
	    	/**
			 * Get the users Total Activity Points/Contributions
			 * @author Aditya Jyoti Saha
			 *
			 */
			function adiGetUserActPoint($userId) {
				echo 'hi';
				...
				//$userIdSql = "SELECT * FROM keylinkz.`keylinkz_user` WHERE user_id = $userId";
				//$userIdRes = mysql_fetch_array(mysql_query($userIdSql));
				//$userIdRes['user_id'];

				$userActPointSql = "SELECT * FROM  `keylinkz_user_activity` WHERE user_id = $userId";
