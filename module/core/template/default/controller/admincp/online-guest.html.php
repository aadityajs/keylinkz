...
{foreach from=$aGuests key=iKey item=aGuest}
	<tr class="checkRow{if is_int($iKey/2)} tr{else}{/if}">
		<td><a href="{url link='admincp.core.ip' search=$aGuest.ip_address_search}" title="{phrase var='admincp.view_all_the_activity_from_this_ip'}">{$aGuest.ip_address}</a></td>
		<td>{$aGuest.user_agent}</td>
		<td class="t_center">
