...
?>
						...
						<ul>
							{foreach from=$aMenus key=iKey item=aMenu name=menu}
								<li {if ($aMenu.url == 'apps' && count($aInstalledApps)) || (isset($aMenu.children) && count($aMenu.children))}class="explore"{/if}><a href="{url link=$aMenu.url}" class="{if isset($aMenu.external) && $aMenu.external == true}no_ajax_link {/if}ajax_link">
