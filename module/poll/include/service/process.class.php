 ...
 * @author  		Raymond Benc
 * @package  		Module_Poll
 * @version 		$Id: process.class.php 3661 2011-12-05 15:42:26Z Miguel_Espinoza $
 */
class Poll_Service_Process extends Phpfox_Service 
{
	...
	public function add($iUser, $aVals, $bIsUpdate = false)
	{
		...
		$aInsert = array(			
			'question' => Phpfox::getLib('parse.input')->clean($aVals['question'], 255),
	...
	public function moderatePoll($iPoll, $iResult)
	{
		...
		$aPoll = $this->database()->select('p.poll_id, p.view_id, p.user_id, p.image_path, p.question, p.privacy, p.privacy_comment')
			->from($this->_sTable, 'p')
		...
		{			
			...
			Phpfox::getLib('mail')->to($aPoll['user_id'])
				->subject(Phpfox::getPhrase('poll.your_poll_title_has_been_approved', array('title' => $aPoll['question'])))
	...
	public function deleteImage($iPoll, $iUser)
	{
		...
		if (!empty($sFileName))
		{
			...
			return $this->database()->update(Phpfox::getT('poll'), array('image_path' => ''), 'poll_id = ' . $iPoll);
		}
		return true;
	...
	{		
		...
		else
		{
			...
			Phpfox::getLib('mail')->to($aPoll['user_id'])
				->subject(Phpfox::getPhrase('poll.full_name_voted_on_your_poll_question', array('full_name' => Phpfox::getUserBy('full_name'), 'question' => $aPoll['question'])))
				->message(Phpfox::getPhrase('poll.full_name_voted_answer_on_your_poll_question', array('full_name' => Phpfox::getUserBy('full_name'), 'answer' => $aPoll['answer'], 'question' => $aPoll['question'], 'link' => $sLink)))
				->send();
