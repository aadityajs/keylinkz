 ...
 * @author  		Raymond Benc
 * @package  		Module_Poll
 * @version 		$Id: add.html.php 3684 2011-12-06 10:47:32Z Miguel_Espinoza $
 */
...
<form method="post" action="{if $bIsCustom}#{else}{url link='poll.add'}{if isset($aForms.poll_id)}id_{$aForms.poll_id}{/if}{/if}" name="js_poll_form" id="js_poll_form" onsubmit="{if $bIsCustom}if ({$sGetJsForm}) {left_curly} $(this).ajaxCall('poll.addCustom'); {right_curly} return false;{else}{$sGetJsForm}{/if}" {if Phpfox::getUserParam('poll.poll_can_upload_image')}enctype="multipart/form-data"{/if}>
	...
	<div id="hideChoices" style="display:none;">
		...
		<div class="table">
			...
			<div class="table_right">				
				<div class="item_is_active_holder">		
					<span class="js_item_active item_is_active"><input type="radio" name="val[randomize]" value="1" class="checkbox" onclick="$('.js_arrow_up_down').hide();" style="vertical-align:middle;"{value type='checkbox' id='randomize' default='1'}/> {phrase var='poll.yes'}</span>
					<span class="js_item_active item_is_not_active"><input type="radio" name="val[randomize]" value="0" class="checkbox" onclick="$('.js_arrow_up_down').show();" style="vertical-align:middle;"{value type='checkbox' id='randomize' default='0' selected=true}/> {phrase var='poll.no'}</span>
				</div>			
			</div>
		</div>	
