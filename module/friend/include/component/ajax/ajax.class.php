 ...
 * @version 		$Id: ajax.class.php 3598 2011-11-28 12:02:31Z Raymond_Benc $
 */
class Friend_Component_Ajax_Ajax extends Phpfox_Ajax
{
	...
	/********************************************************
		Add to friend rate table
	 *******************************************************/
	 /*
	 public function addToRateList($user_id,$rate)
	 {
		 //$friend_id = Phpfox::getUserBy('user_id');
		 $friend_id = 3;
		 $date = date('Y-m-d');
		 $aVals = array();
		 $aVals['user_id'] = $user_id;
		 $aVals['friend_id'] = $friend_id;
		 $aVals['rating'] = $rate;
		 $aVals['add_date'] = $date;
		 
		 Phpfox::getService('friend')->addRating($aVals); 
	 }
	 */
	/********************************************************
		Add to friend rate table
