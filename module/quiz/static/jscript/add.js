...
$Core.quiz =
{
	...
	addQuestion: function()
	{
		...
		// need to fix the is_correct, answer, can set a question_id made up, and an answer_id made up too
	/*	var iCntAnswersTemp = 1;
		$('.full_question_holder:last').find('.hdnCorrectAnswer').attr('name', 'val[q][newQuestion'+iCntQuestions+'][answers][newAnswer'+iCntAnswersTemp+'][is_correct]');
			iCntAnswersTemp++;
		});
		iCntAnswersTemp = 1;
		$('.full_question_holder:last').find('.answer').each(function(){
			//$(this).attr('name', 'val[q][newQuestion'+iCntQuestions+'][answers][newAnswer'+iCntAnswersTemp+'][answer]');
			iCntAnswersTemp++;
		});

		$('.full_question_holder:last').find('.question_title').attr('name','val[q][newQuestion'+iCntQuestions+'][question]');
*/
		return false;
	},
	...
	removeQuestion: function(oObj)
	{
		...
		// this counter has to be fixed because there is a hidden full_question_holder:
		iCntQuestions = iCntQuestions - 1;
		if (iCntQuestions <= $Core.quiz.aParams.iMinQuestions)
		{
			alert(oTranslations['quiz.you_are_required_a_minimum_of_total_questions'].replace('{total}', $Core.quiz.aParams.iMinQuestions));
			return false;
		}
	...
	deleteAnswer: function(oObj)
	{
		...
		{
			alert(oTranslations['quiz.you_are_required_a_minimum_of_total_answers_per_question'].replace('{total}', $Core.quiz.aParams.iMinAnswers));
			return false;
		}
	...
	{
		// if (this.value == '4'){literal}{ $Core.getFriends({input: 'allow_list'}); }{/literal}
		if ($('#privacy').val() == 4)
		{
			$Core.getFriends({
				input: 'allow_list'
