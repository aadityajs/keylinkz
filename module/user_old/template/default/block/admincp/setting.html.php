 ...
 * @author  		Raymond Benc
 * @package 		Phpfox
 * @version 		$Id: setting.html.php 2588 2011-05-03 09:18:09Z Miguel_Espinoza $
 */
...
?>
	{foreach from=$aSettings item=aProduct}
		{foreach from=$aProduct key=sKey item=aSetting}
			...
			<div class="{if is_int($phpfox.iteration.settings/2)}table1{else}table2{/if}{if $aItem.is_admin_setting} is_admin_setting{/if}">
				...
				<div class="table_right2">				    
					...
					<div class="item_is_active_holder">		
						<span class="js_item_active item_is_active"><input type="radio" name="val[value_actual][{$aItem.setting_id}]" value="1" {if $aItem.value_actual}checked="checked" {/if}/> {phrase var='user.yes'}</span>
						<span class="js_item_active item_is_not_active"><input type="radio" name="val[value_actual][{$aItem.setting_id}]" value="0" {if !$aItem.value_actual}checked="checked" {/if}/> {phrase var='user.no'}</span>
					</div>
