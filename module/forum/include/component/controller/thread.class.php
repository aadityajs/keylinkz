...
class Forum_Component_Controller_Thread extends Phpfox_Component
{
	...
	{		
		...
		if ($iPostId = $this->request()->getInt('post'))
		{
			...
			$this->url()->permalink('forum.thread', $aThread['thread_id'], $aThread['title'], true, null, array('page' => $iCurrentPage, '#post' . $iPostId));
		}			
		...
		if ($aCallback === null)
		{
			$this->template()->setBreadcrumb(Phpfox::getPhrase('forum.forum'), $this->url()->makeUrl('forum'))
				->setBreadcrumb($aForum['breadcrumb'])->setBreadcrumb($aForum['name'], $this->url()->permalink('forum', $aForum['forum_id'], $aForum['name']));
		}	
		else 
		{
		...
		{
			if (Phpfox::getService('pages')->isAdmin(''))
			{
				$bCanEditThread = true;
				$bCanDeleteThread = true;
				$bCanStickThread = true;
