	...
	public function process()
	{
		...
		if ($oSearch->isSearch())
		{
			...
			if ((!count($aSearchResults)) || (count($aSearchResults) && !$oSearch->cacheResults(array('keyword', 'user'), $aSearchResults)))
			{
				if (!empty($aSearch['group_id']))
				{
					$this->url()->send('forum.search', array('module' => 'group', 'item' => $aSearch['group_id']), Phpfox::getPhrase('forum.no_results_found'));
				}
				else 
				{
		...
		{			
			...
			if ($bIsTagSearch)
			{
				if ($bIsModuleTagSearch)
				{
					$this->template()->setBreadcrumb(Phpfox::getPhrase('forum.threads_tagged_with') . ': ' . $this->request()->get('req5'), $this->url()->makeUrl('forum.tag.module_group.item_' . $this->request()->get('item') . '.'  . $this->request()->get('req5')), true);
				}
				else 
				{
					$this->template()->setBreadcrumb(Phpfox::getPhrase('forum.tags'), $this->url()->makeUrl('forum.tag'))->setBreadcrumb(Phpfox::getPhrase('forum.threads_tagged_with') . ': ' . $this->request()->get('req3'), $this->url()->makeUrl('forum.tag.'  . $this->request()->get('req3')), true);
				}
			}
			else 
			{
				if (is_array($aCallback))
				{
					$this->template()->setBreadcrumb(Phpfox::getPhrase('forum.search'), $this->url()->makeUrl('forum.search', array('module' => 'group', 'item' => $aCallback['group_id'])));
				}
				else 
				{
		...
		{			
			...
			{			
				...
				$this->template()->setTitle($aForum['name'])
					->setBreadcrumb($aForum['breadcrumb'])
					->setBreadcrumb($aForum['name'], $this->url()->permalink('forum', $aForum['forum_id'], $aForum['name']), true)
					->assign(array(
