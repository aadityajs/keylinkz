 ...
 * @version 		$Id: ajax.class.php 3737 2011-12-09 07:50:12Z Raymond_Benc $
 */
class Forum_Component_Ajax_Ajax extends Phpfox_Ajax
{
	...
	public function deleteThread()
	{
		...
		if ($bHasAccess)
		{
			...
			if ((int) $aThread['group_id'] > 0)
			{
				$this->call('window.location.href = \'' . Phpfox::getLib('url')->makeUrl('group', array('redirect', 'id' => $aThread['group_id'])) . '\';');	
			}
			else 
			{
	...
	public function savePerms()
	{
		Phpfox::isUser(true);
		Phpfox::getUserParam('forum.can_manage_forum_permissions', true);
		Phpfox::getService('forum.process')->savePerms($this->get('val'));
	}
