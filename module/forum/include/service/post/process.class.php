...
class Forum_Service_Post_Process extends Phpfox_Service 
{
	...
	public function add($aVals, $aCallback = false, $aExtra = array())
	{
		...
		{		
			...
			if (!Phpfox::getService('forum')->isPrivateForum($aThread['forum_id']))
			{
				...
				{
					Phpfox::getService('feed.process')->callback($aCallback)->add('forum_reply', $iId, 0, 0, ($aCallback === false ? 0 : $aCallback['item']));
				}
			}
		}
	...
	public function approve($iPostId)
	{
		...
		((Phpfox::isModule('feed') && !defined('PHPFOX_SKIP_FEED_ENTRY')) ? Phpfox::getService('feed.process')->add('forum_reply', $iPostId, 0, 0, 0, $aPost['user_id']) : null);		
	...
	public function delete($iPostId)
	{
		...
		Phpfox::getService('feed.process')->delete('forum_reply', $iPostId);
