 ...
 * @version 		$Id: forum.class.php 3342 2011-10-21 12:59:32Z Raymond_Benc $
 */
class Forum_Service_Forum extends Phpfox_Service 
{
	...
	private function _getBreadcrumb($iId)
	{
		...
		{			
			...
			$this->_aBreadcrumbs[] = array($this->_aForums[$iId]['name'], Phpfox::getLib('url')->permalink('forum', $this->_aForums[$iId]['forum_id'], $this->_aForums[$iId]['name']));		
		}
	}
	...
	private function _buildAdminCp($iForumId)
	{
		...
		{			
			...
			$sHtml .= '<li><img src="' . Phpfox::getLib('template')->getStyle('image', 'misc/draggable.png') . '" alt="" /> <input type="hidden" name="order[' . $aForum['forum_id'] . ']" value="' . $iCnt . '" /> <a href="#?id=' . $aForum['forum_id'] . '" class="js_drop_down">' . $aForum['name'] . '</a>';
	...
	{		
		...
		{			
			...
			$sOptions .= '<option value="' . ($bIdOnly ? $aForum['forum_id'] : Phpfox::getLib('url')->permalink('forum', $aForum['forum_id'], $aForum['name'])) . '"' . ($this->_iActive == $aForum['forum_id'] ? ' selected="selected"' : '') . '>' . $sExt . $aForum['name'] . '</option>' . "\n";
		...
		{			
			...
			{				
				...
				{					
					...
					elseif ((PHPFOX_TIME - Phpfox::getParam('forum.keep_active_posts') * 60) > $aForum['thread_time_stamp'])					
					{						
						$aForum['is_seen'] = 1;							
					}				
