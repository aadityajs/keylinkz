 ...
 * @version 		$Id: process.class.php 3880 2012-01-24 09:59:41Z Raymond_Benc $
 */
class Forum_Service_Thread_Process extends Phpfox_Service 
{
	...
	public function update($iId, $iUserId, $aVals)
	{
		...
		$this->database()->update(Phpfox::getT('forum_post'), array(
				'total_attachment' => Phpfox::getService('attachment')->getCountForItem($aVals['post_id'], 'forum'),
				'title' => $oParseInput->clean($aVals['title'], 255),
	...
	public function delete($iThread)
	{
		...
		{				
			$this->database()->update(Phpfox::getT('forum'), array('thread_id' => $aLastThread['thread_id'], 'post_id' => $aLastThread['post_id']), 'thread_id = ' . $aThread['thread_id']);
		}
		else 
		{
			if ($aThread['forum_id'] > 0)
			{
				$this->database()->update(Phpfox::getT('forum'), array('thread_id' => 0, 'post_id' => 0), 'thread_id = ' . $aThread['forum_id']);	
			}			
		}		
		...
		foreach ($aPosts as $aPost)
		{
			...
			(Phpfox::isModule('feed') ? Phpfox::getService('feed.process')->delete('forum_reply', $aPost['post_id']) : null);
		}
	...
	{		
		...
		$aOtherThread = $this->database()->select('ft.*')
			->from($this->_sTable, 'ft')
			->where('ft.group_id = ' . ($bIsGroup ? $aThread['group_id'] : '0') . ' AND ft.view_id = 0 AND ft.title_url = \'' . $this->database()->escape($aParams[($bIsGroup ? 'req4' : 'req3')]) . '\'')			
			->execute('getRow');		
		...
		{
			return Phpfox::getLib('url')->makeUrl('group.forum', array($aOldThread['title_url'], 'id' => $aThread['group_id']));
		}
	}
	...
	public function approve($iThreadId)
	{
		...
		((Phpfox::isModule('feed') && !defined('PHPFOX_SKIP_FEED_ENTRY')) ? Phpfox::getService('feed.process')->add('forum', $iThreadId, 0, 0, 0, $aThread['user_id']) : null);
		...
		if (isset($aVals['type_id']))
		{
			switch ($aVals['type_id'])
			{
				case 'sticky':
					...
					if ($aCallback !== false)
					{
						if (Phpfox::isModule('group') && Phpfox::getService('group')->isAdmin($aCallback['group_id']))
						{
							$bHasAccess = true;	
						}
					}
