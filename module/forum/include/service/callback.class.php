...
class Forum_Service_Callback extends Phpfox_Service 
{
	...
	{
		Phpfox::getService('notification.process')->delete('forum_reply_notifyLike', $iChildId, Phpfox::getUserId());
	...
	public function addLikePost($iItemId, $bDoNotSendEmail = false)
	{
		...
		if (!$bDoNotSendEmail)
		{
			...
			Phpfox::getLib('mail')->to($aRow['user_id'])
				->subject(Phpfox::getPhrase('forum.full_name_liked_one_of_your_forum_posts', array('full_name' => Phpfox::getUserBy('full_name'))))
				->message(Phpfox::getPhrase('forum.full_name_liked_your_one_of_your_forum_posts_in', array('full_name' => Phpfox::getUserBy('full_name'), 'link' => $sLink, 'title' => $aRow['title'])))
	...
	public function getActivityFeedReply($aItem)
	{
		if (Phpfox::isUser())
		{
	...
	{		
		$aRow = $this->database()->select('ft.thread_id, ft.title, ft.group_id, ft.user_id, fp.total_like, fpt.text_parsed AS text, ft.time_stamp, l.like_id AS is_liked, fp.post_id')
			->from(Phpfox::getT('forum_thread'), 'ft')
		...
		return array(
			...
			'feed_info' => Phpfox::getPhrase('feed.posted_a_thread'),
			'feed_link' => $sLink,
			'feed_content' => $aRow['text'],
			'feed_total_like' => $aRow['total_like'],
			'feed_is_liked' => isset($aRow['is_liked']) ? $aRow['is_liked'] : false,
