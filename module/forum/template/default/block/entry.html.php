...
<div class="table_info">
	<b>{phrase var='forum.sub_forum'}:</b> {$aForumData.name|clean}
	{if !empty($aForumData.description)}
	<div class="p_4">
		{$aForumData.description|parse}
	</div>		
	{/if}
</div>
{/if}
...
<div class="table_info">
	<a href="{permalink module='forum' id=$aForum.forum_id title=$aForum.name}"{if !empty($aForum.description)} title="{$aForum.description|parse}" {/if}>{$aForum.name|clean}</a>	
</div>
