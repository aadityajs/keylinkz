 ...
 * @version 		$Id: process.class.php 3743 2011-12-09 10:29:12Z Miguel_Espinoza $
 */
		...
		{		
			...
			if (isset($aUser['user_id']) && $aUser['user_id'] != $aPhoto['user_id'])
			{
				...
				Phpfox::getLib('mail')->to($iTagUserId)
					->subject('' . Phpfox::getUserBy('full_name') . ' tagged you in a photo.')
					->message((Phpfox::getUserId() == $aPhoto['user_id'] ? '' . Phpfox::getUserBy('full_name') . ' tagged you on  ' . Phpfox::getService('user')->gender(Phpfox::getUserBy('gender'), 1) . ' photo "<a href="' . $sLink . '">' . $aPhoto['title'] . '</a>".' : '' . Phpfox::getUserBy('full_name') . ' tagged you on ' . $aPhoto['full_name'] . '\'s photo "<a href="' . $sLink . '">' . $aPhoto['title'] . '</a>".') . "\nTo view this photo follow the link below:\n<a href=\"" . $sLink . "\">" . $sLink . "</a>")
					->send();	
