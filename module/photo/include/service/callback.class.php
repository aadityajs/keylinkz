...
class Photo_Service_Callback extends Phpfox_Service 
{
	...
	public function getActivityFeedComment($aRow)
	{
		...
		$aItem = $this->database()->select('b.photo_id, b.server_id, b.destination, b.title, b.time_stamp, b.privacy, b.total_comment, b.total_like, c.total_like, ct.text_parsed AS text, f.friend_id AS is_friend, ' . Phpfox::getUserField())
	...
	public function addLike($iItemId, $bDoNotSendEmail = false)
	{
		...
		if (!$bDoNotSendEmail)
		{
			...
			Phpfox::getLib('mail')->to($aRow['user_id'])
				->subject(Phpfox::getPhrase('custom.full_name_liked_your_photo_title', array('full_name' => Phpfox::getUserBy('full_name'), 'title' => $aRow['title'])))
				->message(Phpfox::getPhrase('custom.full_name_liked_your_photo_message', array('full_name' => Phpfox::getUserBy('full_name'), 'link' => $sLink, 'title' => $aRow['title'])))
	...
	public function getActivityFeed($aItem, $aCallback = null)
	{		
		if ($aCallback === null)
		{
		...
		if ($aRow['extra_photo_id'] > 0)
		{
			...
			{		
				...
				{
					$aListPhotos[] = '<a href="' . Phpfox::permalink('photo', $aPhoto['photo_id'], $aPhoto['title']) . ($bIsPhotoAlbum ? 'albumid_' . $aRow['album_id'] : 'userid_' . $aRow['user_id']) . '/" class="thickbox photo_holder_image" rel="' . $aPhoto['photo_id'] . '">' . Phpfox::getLib('image.helper')->display(array(
			...
			$aListPhotos = array_merge($aListPhotos, array('<a href="' . Phpfox::permalink('photo', $aRow['photo_id'], $aRow['title']) . ($bIsPhotoAlbum ? 'albumid_' . $aRow['album_id'] : 'userid_' . $aRow['user_id']) . '/" ' . (empty($sFeedImageOnClick) ? ' class="thickbox photo_holder_image" rel="' . $aRow['photo_id'] . '" ' : $sFeedImageOnClick . ' class="no_ajax_link"') . '>' . $sImage . '</a>'));
		}
	...
	public function addLikeAlbum($iItemId, $bDoNotSendEmail = false)
	{
		...
		if (!$bDoNotSendEmail)
		{
			...
			Phpfox::getLib('mail')->to($aRow['user_id'])
				->subject(Phpfox::getPhrase('custom.full_name_liked_your_photo_album_name', array('full_name' => Phpfox::getUserBy('full_name'), 'name' => $aRow['name'])))
				->message(Phpfox::getPhrase('custom.full_name_liked_your_photo_album_message', array('full_name' => Phpfox::getUserBy('full_name'), 'link' => $sLink, 'name' => $aRow['name'])))
				->notification('like.new_like')
				->send();
	...
	public function getActivityFeedTag($aItem, $aCallback = null)
	{
		...
		{
			$sPhrase = 'tagged ' . Phpfox::getService('user')->gender($aItem['gender'], 2) . ' on <a href="' . Phpfox::getLib('url')->makeUrl($aRow['photo_user_name']) . '">' . $aRow['photo_full_name'] . '</a>\'s photo.';
		}
		else 
		{
