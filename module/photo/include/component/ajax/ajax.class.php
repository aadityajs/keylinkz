...
class Photo_Component_Ajax_Ajax extends Phpfox_Ajax
{
    ...
    public function getCategoryForEdit()
    {
		...
		$this->html('#js_photo_table_header', Phpfox::getPhrase('photo.editing_category') . ': ' . $aCategory['name'])
			...
			->html('#js_photo_extra_button', ' <input type="button" name="" value="' . Phpfox::getPhrase('photo.cancel') . '" class="button" onclick="$(\'#js_photo_category_' . $aCategory['parent_id'] . '\').attr(\'selected\', false); $(\'#js_category_holder\').show(); $(\'#js_photo_table_header\').html(\'' . Phpfox::getPhrase('music.add_a_photo_category') . '\'); $(\'#js_photo_extra_button\').html(\'\'); $(\'#js_photo_hidden\').html(\'\'); $(\'#name\').val(\'\');" /> <input type="submit" value="' . Phpfox::getPhrase('photo.delete') . '" onclick="return confirm(\'' . Phpfox::getPhrase('photo.are_you_sure') . '\');" class="button" name="val[delete]" />')
    ...
    public function process()
    {
		...
		else
		{
		    ...
		    $this->call('$.ajaxCall(\'photo.process\', \'&action=' . $this->get('action') . '&js_disable_ajax_restart=true&photos=' . urlencode(base64_encode(serialize($aImages))) . $sExtra . '\');');
		}
    }
	...
	public function massUpdate()
	{
		...
		{
			$aPhoto = Phpfox::getLib('database')->select('photo_id, user_id')
