 ...
 * @version 		$Id: frame.class.php 3296 2011-10-12 13:29:57Z Raymond_Benc $
 */
class Photo_Component_Controller_Frame extends Phpfox_Component
{
	...
	{		
		...
		if (count($aImages))
		{
			...
			{
				echo '<script type="text/javascript">';
				echo 'window.parent.$.ajaxCall(\'photo.process\', \'js_disable_ajax_restart=true&twitter_connection=' . ((isset($aVals['connection']) && isset($aVals['connection']['twitter'])) ? $aVals['connection']['twitter'] : '0') . '&facebook_connection=' . (isset($aVals['connection']['facebook']) ? $aVals['connection']['facebook'] : '0') . '&custom_pages_post_as_page=' . $this->request()->get('custom_pages_post_as_page') . '&photos=' . urlencode(base64_encode(serialize($aImages))) . '&action=' . $sAction . '' . (isset($iFeedId) ? '&feed_id=' . $iFeedId : '') . '' . ($aCallback !== null ? '&callback_module=' . $aCallback['module'] . '&callback_item_id=' . $aCallback['item_id'] : '') . '&parent_user_id=' . (isset($aVals['parent_user_id']) ? (int) $aVals['parent_user_id'] : 0) . '\');';
				echo '</script>';
			}
