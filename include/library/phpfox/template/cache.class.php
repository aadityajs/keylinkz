 ...
 * @package 		Phpfox
 * @subpackage 		Template
 * @version 		$Id: cache.class.php 3882 2012-01-24 11:38:37Z Raymond_Benc $
 */
class Phpfox_Template_Cache extends Phpfox_Template
{
	...
	public function compile($sName, $sData = null, $bRemoveHeader = false, $bSkipDbCheck = false)
	{
		...
		{
			if (preg_match("/(.*?)template_(.*?)_(.*?)_template_(.*?)\.php$/i", str_replace('template/', 'template_', $sName), $aMatches) && isset($aMatches) && $aMatches[2] == 'frontend')
			{
			...
			elseif ($bSkipDbCheck === false && preg_match("/(.*?)template_(.*?)_template_(.*?)_(.*?)_(.*?)\.php$/i", str_replace('template/', 'template_', $sName), $aMatches) && isset($aMatches[5]) && !preg_match("/admincp_(.*?)/", $aMatches[4]))
			{
				$oDb = Phpfox::getLib('database');
	...
	private function _parseFunction($sFunction, $sModifiers, $sArguments)
	{
		switch ($sFunction)
		{
			...
			case 'block':
				...
				$sContent .= '<?php else: ?>';
				$sContent .= '<div class="sample"<?php echo (!defined(\'PHPFOX_NO_WINDOW_CLICK\') ? " onclick=\"window.parent.$(\'#location\').val(' . $aArgs['location'] . '); window.parent.tb_remove();\"" : \' style="cursor:default;"\'); ?>><?php echo Phpfox::getPhrase(\'core.block\') ; ?> ' . $this->_removeQuote($aArgs['location']) . '';
				$sContent .= '<?php if (defined(\'PHPFOX_IS_AD_SAMPLE\')): Phpfox::getBlock(\'ad.sample\', array(\'block_id\' => ' . $this->_removeQuote($aArgs['location']) . ')); endif; ?>';
				$sContent .= '</div>';
				$sContent .= '<?php endif; ?>';
				$sContent .= '<?php else: ?>';
			...
			case 'select_location':
				...
				$sContries = '<select name="val[' . (isset($aArgs['name']) ? $this->_removeQuote($aArgs['name']) : 'country_iso') . ']" id="' . (isset($aArgs['name']) ? $this->_removeQuote($aArgs['name']) : 'country_iso') . '"' . (isset($aArgs['style']) ? ' style=' . $aArgs['style'] . '' : '') . '>' . "\n";
			...
			else
			{
				switch ($sMod)
				{
					...
					case 'user':
						...
						$sValue = '\' . ' . $sVariable . '[\'' . $sSuffix . 'full_name\'] . \'';
						if (count($aArgs))
						{
							if (!empty($aArgs[1]))
							{
							...
							if (!empty($aArgs[3]))
							{
								$sValue = '\' . Phpfox::getLib(\'phpfox.parse.output\')->split(' . $sVariable . '[\'' . $sSuffix . 'full_name\'], ' . $this->_removeQuote($aArgs[3]) . '' . (empty($aArgs[3]) ? '' : ', true') . ') . \'';
							}
						}
