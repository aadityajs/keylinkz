 ...
 * @author			Raymond Benc
 * @package 		Phpfox
 * @version 		$Id: helper.class.php 3533 2011-11-21 14:07:21Z Raymond_Benc $
 */
...
{	
	...
	public function display($aParams)
	{
		...
		if (isset($aParams['user']))
		{
			...
			$aParams['title'] = ($bIsOnline ? Phpfox::getPhrase('core.full_name_is_online', array('full_name' => $aParams['user'][$sSuffix . 'full_name'])) : $aParams['user'][$sSuffix . 'full_name']);
		...
		if ($bIsServer === true)
		{
			...
			{				
				$sSrc = Phpfox::getLib('cdn')->getUrl($sSrc);	
			}
			else 
			{
